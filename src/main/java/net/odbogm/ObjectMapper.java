...
public class ObjectMapper {
    ...
    public void fillSequenceFields(Object o, ClassDef classdef, Transaction t) {
        if (!classdef.sequenceFields.isEmpty()) {
            ...
            OSequenceLibrary seqLibrary = t.getODatabaseSession().getMetadata().getSequenceLibrary();
     ...
     */
    public <T> T hydrate(Class<T> c, OElement v, Transaction t) throws DuplicateClassDefinition, InstantiationException, IllegalAccessException, NoSuchFieldException, CollectionNotSupported {
        t.initInternalTx();
        LOGGER.log(Level.FINER, "class: {0}  vertex: {1}", new Object[]{c, v});

        Class<?> toHydrate = c;
        String entityClass = ClassCache.getEntityName(toHydrate);
        String vertexClass = (v.getSchemaType().get().getName().equals("V") ? entityClass : v.getSchemaType().get().getName());
        ...
        t.transactionLoopCache.put(v.getIdentity(), proxy);
        ...
        for (Map.Entry<String, Class<?>> entry : classdef.linkLists.entrySet()) {
            try {
                ...
                ODirection RelationODirection = ODirection.OUT;

                // si hay Vértices conectados o si el constructor del objeto ha inicializado los vectores, convertirlos
                if ((VertexUtils.getEdgeCountOfVertex((OVertex)v, RelationODirection, graphRelationName) > 0) || (fLink.get(proxy) != null)) {
                    this.colecctionToLazy(proxy, field, fc, (OVertex)v, t);
                }
        ...
        for (Map.Entry<String, Class<?>> entry : classdef.indirectLinkLists.entrySet()) {
            try {
                ...
                ODirection RelationODirection = ODirection.IN;
     ...
     *
     */
    public void colecctionToLazy(Object o, String field, Class<?> fc, OElement v, Transaction t) {
        LOGGER.log(Level.FINER, "***************************************************************");
        ...
        try {
            ...
            if (col instanceof List) {
                ...
                ((ILazyCollectionCalls) col).init(t, (OVertex)v, (IObjectProxy) o,
                        graphRelationName, listClass, oDirection);
                ...
                // inicializar la colección
                ((ILazyMapCalls) col).init(t, (OVertex)v, (IObjectProxy) o,
                        graphRelationName, keyClass, valClass, oDirection);
            } else {
